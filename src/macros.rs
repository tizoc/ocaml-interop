...
macro_rules! ocaml {
    ...
    ($vis:vis fn $name:ident($arg:ident: $typ:ty) -> $rtyp:ty; $($t:tt)*) => {
        $vis unsafe fn $name(token: $crate::GCToken, $arg: $crate::OCaml<$typ>) -> Result<$crate::GCResult<$rtyp>, $crate::Error> {
    ...
    ($vis:vis fn $name:ident($arg1:ident: $typ1:ty, $arg2:ident: $typ2:ty) -> $rtyp:ty; $($t:tt)*) => {
        $vis unsafe fn $name(token: $crate::GCToken, $arg1: $crate::OCaml<$typ1>, $arg2: $crate::OCaml<$typ2>) -> Result<$crate::GCResult<$rtyp>, $crate::Error> {
            $crate::ocaml_closure_reference!(F, $name);
    ...
    ($vis:vis fn $name:ident($arg1:ident: $typ1:ty, $arg2:ident: $typ2:ty, $arg3:ident: $typ3:ty) -> $rtyp:ty; $($t:tt)*) => {
        $vis unsafe fn $name(token: $crate::GCToken, $arg1: $crate::OCaml<$typ1>, $arg2: $crate::OCaml<$typ2>, $arg3: $crate::OCaml<$typ3>) -> Result<$crate::GCResult<$rtyp>, $crate::Error> {
            $crate::ocaml_closure_reference!(F, $name);
...
macro_rules! ocaml_closure_reference {
    ($var:ident, $name:ident) => {
        ...
        static mut OC: Option<$crate::OCamlClosure> = None;
        if OC.is_none() {
            OC = $crate::OCamlClosure::named(name);
        }
        let $var = OC.unwrap_or_else(|| panic!("OCaml closure with name '{}' not registered", name));
    };
}
