...
mod tests {
    ...
    fn example_inspect_unit() {
        let inspection = unsafe { 
            inspect_raw_value(ocaml_sys::UNIT)
        };
    ...
    fn example_inspect_boolean() {
        let true_inspection = unsafe { 
            inspect_raw_value(ocaml_sys::TRUE)
        };
        
        let false_inspection = unsafe { 
            inspect_raw_value(ocaml_sys::FALSE)
        };
...
fn main() {
    ...
    let int_inspection = unsafe { 
        inspect_raw_value((123_isize << 1) | 1)
    };
